        ...
        boost::asio::async_read(*socket,
            ...
            [this, self, socket, header_buf](boost::system::error_code ec, size_t bytes) {
                ...
                if (payload_length > 0) {
                    ...
                    // 保持 socket 活跃，直到 payload 读取完成，避免服务端提前 RST
                    boost::asio::async_read(*socket,
                        boost::asio::buffer(payload_buf->data(), payload_length),
                        [this, self, socket, payload_buf](boost::system::error_code read_ec, size_t read_bytes) {
                            if (read_ec) {
                                self->malformed_count_++;
                            } else if (read_bytes == payload_buf->size()) {
    ...
    boost::asio::co_spawn(io_ctx,
        [conn, port]() -> boost::asio::awaitable<void> {
            ...
            co_await conn->asyncSend(nullptr, 0,
                static_cast<uint32_t>(ControlMessageType::Unknown), 1);
        },
        boost::asio::detached);
