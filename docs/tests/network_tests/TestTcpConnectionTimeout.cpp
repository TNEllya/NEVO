...
protected:
    ...
    void runIoContext(std::chrono::milliseconds duration = std::chrono::seconds(10)) {
        ...
        // 阻塞等待 io_context 运行结束，确保所有异步操作完成后再做断言
        io_thread_.join();
    }
...
TEST_F(TcpConnectionTimeoutTest, ConnectSuccess) {
    auto acceptor = std::make_shared<boost::asio::ip::tcp::acceptor>(*io_ctx_,
        boost::asio::ip::tcp::endpoint(boost::asio::ip::tcp::v4(), 0));
    uint16_t port = acceptor->local_endpoint().port();

    // 启动一个协程负责接受一次连接，确保 ConnectSuccess 真的有服务端在监听
    boost::asio::co_spawn(*io_ctx_,
        [acceptor]() -> boost::asio::awaitable<void> {
            auto peer = co_await acceptor->async_accept(boost::asio::use_awaitable);
    ...
    // 先让 acceptor 协程把 async_accept 挂起，避免 connect 先执行导致 connection refused
    io_ctx_->run_one();
...
TEST_F(TcpConnectionTimeoutTest, MultipleRapidConnections) {
    ...
    // 先让 acceptor 协程把 async_accept 挂起
    io_ctx_->run_one();
