...
constexpr float kResamplerTolerance = 0.1f;
constexpr uint32_t kTransientSamples = 10;
...
TEST(ResamplerTest, Upsample16kTo48k) {
    ...
    for (uint32_t i = kTransientSamples; i < output_frames; ++i) {
        EXPECT_NEAR(output[i], 0.5f, kResamplerTolerance);
    }
}
...
TEST(ResamplerTest, FullScaleInput) {
    ...
    uint32_t output_frames = result.value();
    for (uint32_t i = kTransientSamples; i < output_frames; ++i) {
        EXPECT_NEAR(output[i], 1.0f, kResamplerTolerance);
    }
}
