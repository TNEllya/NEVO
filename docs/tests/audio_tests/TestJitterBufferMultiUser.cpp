...
TEST_F(JitterBufferMultiUserTest, RemoveNonexistentUserIsSafe) {
    ...
    uint8_t* out_data = nullptr;
    uint32_t out_size = 0;
    EXPECT_FALSE(jb->pop(UserId(999), out_data, out_size));
}
...
TEST_F(JitterBufferMultiUserTest, EmptyPacketHandling) {
    ...
    jb->push(UserId(1), &dummy, 0, 1000);

    uint8_t* out_data = nullptr;
    uint32_t out_size = 0;
    // JitterBuffer 忽略空包，因此弹不出数据
    EXPECT_FALSE(jb->pop(UserId(1), out_data, out_size));
}
...
TEST_F(JitterBufferMultiUserTest, UserDataPersistenceAfterPop) {
    ...
    // JitterBuffer pop 后会移除数据，第二次 pop 应失败
    EXPECT_FALSE(jb->pop(UserId(1), out_data2, out_size2));
}
