...
TEST_F(AudioMixerVolumeTest, TwentyUsersConcurrentMixing) {
    ...
    cfg.max_speakers = num_users;
    cfg.frame_size = frame_size;
    AudioMixer large_mixer(cfg);

    large_mixer.reset();
    for (uint32_t i = 0; i < num_users; ++i) {
        large_mixer.addStream(UserId(i), pcm.data(), frame_size);
    }

    EXPECT_EQ(large_mixer.activeStreamCount(), num_users);

    std::vector<float> output(frame_size, 0.0f);
    large_mixer.mix(output.data(), frame_size);
